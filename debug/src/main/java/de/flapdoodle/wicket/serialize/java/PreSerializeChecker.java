 ...
 * found, a {@link WicketSerializableCheckException} is thrown with a message that shows the trace up
 ...
 * {@link PreSerializeChecker#isAvailable()} to see whether this class can operate properly. If it
...
public final class PreSerializeChecker extends ObjectOutputStream
{
	...
	/** log. */
	private static final Logger log = LoggerFactory.getLogger(PreSerializeChecker.class);
	 ...
	 * @throws IOException
	 */
	public PreSerializeChecker(ISerializableCheck serializableCheck) throws IOException
	{
		this.serializableCheck = serializableCheck;
		this.stackTracePrinter = new PrettyStackTracePrinter();
	}
	 ...
	 */
	private final String toPrettyPrintedStack(Class<?> type,String message)
	{
		return stackTracePrinter.printStack(type,message);
	}
