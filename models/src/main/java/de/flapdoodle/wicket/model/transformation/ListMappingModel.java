...
public class ListMappingModel<T, D> extends AbstractReadOnlyDetachDelegationModel<List<D>> implements IReadOnlyListModel<D> {
	...
	private final SerializableFunction<T, D> map;

	public ListMappingModel(IModel<? extends Iterable<? extends T>> source, SerializableFunction<T, D> map) {
		super(source);
		this.source = source;
		this.map = map;
	}
	...
	protected static <T, I extends Iterable<? extends T>, D> List<D> map(I entries, SerializableFunction<T, D> map) {
